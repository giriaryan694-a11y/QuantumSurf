...
FIXED: Scroll up/down buttons + hardware keyboard keys not working
  - Scroll: Uses Runtime.evaluate window.scrollBy() + Input.synthesizeScrollGesture
  - Keyboard: Uses capture-phase keydown listener + stopPropagation
"""
...
class CDPWorker(threading.Thread):
    ...
    def _inject_stealth(self,vw,vh):
        ...
        const fp=[{{name:'Chromium PDF Plugin',filename:'internal-pdf-viewer',description:'Portable Document Format',length:1}},{{name:'Chromium PDF Viewer',filename:'mhjfbmdgcfjbbpaeojofohoefgiehjai',description:'',length:1}},{{name:'Native Client',filename:'internal-nacl-plugin',description:'',length:2}}];
    ...
    # ============================================================
    # FIX: Reliable scroll using JS window.scrollBy() + CDP gesture
    # mouseWheel via dispatchMouseEvent is unreliable in headless [[1]][[5]]
    # ============================================================
    def _scroll_page(self, dy):
        """Scroll the remote page by dy pixels. Positive = scroll down, negative = scroll up.
        Uses 3 methods in order of reliability:
        1. Runtime.evaluate window.scrollBy() — most reliable [[14]][[16]]
        2. Input.synthesizeScrollGesture — good for complex layouts [[15]]
        3. Input.dispatchMouseEvent mouseWheel — fallback with mouse positioning first [[3]]
        """
        # Method 1: JavaScript scrollBy (most reliable in headless)
        try:
            ...
            if resp and "result" in resp:
                ...
                # Also try scrolling any focused/active scrollable element
        ...
        # Method 2: Input.synthesizeScrollGesture (experimental but works well) [[15]]
        try:
            cx,cy=self.vp_width//2,self.vp_height//2
            # yDistance: positive = scroll up, negative = scroll down (opposite of scrollBy)
            self._send_session("Input.synthesizeScrollGesture",{
                "x":cx,"y":cy,
                "yDistance":-dy,  # Invert: scrollBy positive=down, gesture positive=up
                "speed":800,
                "preventFling":True
            },timeout=5)
            return
        ...
        # Method 3: dispatchMouseEvent mouseWheel with mouse positioning first [[3]]
        try:
            cx,cy=self.vp_width//2,self.vp_height//2
            # First move mouse to center (required before wheel events work)
    ...
    def _take_screenshot(self):
        ...
        for attempt in range(2):
            try:
                resp=self._send_session("Page.captureScreenshot",{"format":"jpeg","quality":self.QUALITY,"captureBeyondViewport":False,"optimizeForSpeed":True,"fromSurface":True},timeout=5)
                if resp and "result" in resp:
    ...
    def _exec(self,cmd):
        ...
        if action=="navigate":
            ...
            self._take_screenshot(); threading.Timer(3.0,self._take_screenshot).start()
        elif action=="reload":
            self._page_loaded.clear(); self._load_phase="navigating"
            self._send_session("Page.reload",{},timeout=10)
            self._wait_for_load(timeout=15); self._take_screenshot(); threading.Timer(3.0,self._take_screenshot).start()
            ...
            # ============================================================
            # FIX: scroll_up/scroll_down now use _scroll_page() which
            # uses JS scrollBy + synthesizeScrollGesture + mouseWheel
            # ============================================================
                ...
                if self.is_mobile:
                    ...
                    # FIX: Use _scroll_page for non-mobile touch scroll too
...
# ============================================================
# FIX: JavaScript changes for keyboard + scroll:
# 1. keydown listener uses CAPTURE PHASE (3rd arg true) [[23]]
# 2. Added e.stopPropagation() alongside e.preventDefault() [[22]]
# 3. Feed image is focusable (tabindex=0) and auto-focused on click
# 4. Space bar scrolls page down (like a real browser)
# 5. Scroll buttons use the fixed backend _scroll_page()
# ============================================================
...
FIX: Hardware keyboard listener uses CAPTURE PHASE (3rd arg true)
This intercepts keydown BEFORE it reaches any target element,
ensuring e.preventDefault() actually prevents browser defaults
like arrow-key scrolling, Tab navigation, Space scrolling, etc.
Also added e.stopPropagation() to prevent event bubbling.
============================================================ */
document.addEventListener('keydown',function(e){
    /* Skip if typing in address bar or keyboard input field */
    if(document.activeElement===addr)return;
    if(document.activeElement&&document.activeElement.id==='kb-in')return;
    ...
    /* Keys to forward to remote browser */
    var specialKeys=['Enter','Backspace','Tab','Escape','Delete','Insert',
    ...
    if(specialKeys.includes(e.key)){
        ...
        /* Space bar: scroll page down like a real browser */
        k=' ';
